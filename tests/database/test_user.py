...
def test_find_user_by(mocked_supabase_connection, inject_example_users):
    retrieved_user = mocked_supabase_connection.find_by("users", "username", "testuser2")

    assert retrieved_user == [inject_example_users[1]]


def test_find_user_ilike(mocked_supabase_connection, inject_example_users):
    retrieved_users = mocked_supabase_connection.find_ilike("users", "username", "TESTUSER")
...
def test_update_user(mocked_supabase_connection, inject_example_users):
    ...
    updates = {"role": "admin"}
    mocked_supabase_connection.update_by("users", "id", inject_example_users[0]["id"], updates)

    retrieved_user = mocked_supabase_connection.find_by("users", "id", inject_example_users[0]["id"])
