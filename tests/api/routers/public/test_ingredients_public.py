...
@pytest.mark.asyncio
async def test_get_ingredient_by_search_phrase(
    mock_supabase_connection,
    example_ingredients_injection,
    example_ingredients_response,
):
    response = await get_ingredients(search_phrase="MusHRoom")

    assert response == [example_ingredients_response[2]]

    parsed = IngredientResponse(**response[0])
...
async def test_get_ingredients_by_search_phrase(
    ...
    parsed = [IngredientResponse(**item) for item in response]

    for item in parsed:
        assert isinstance(item, IngredientResponse)
