import pytest

from api.crud.single_entity.post_methods import create_element
from api.crud.nested.post_methods import create_nested
from api.crud.nested.get_methods import get_nested


ingredient_create = {
    "name": "Carrot",
}

ingredient_data_create = [{
    "ingredients_data": 
    {
        "calories_per_100": 41.6,
        "protein_per_100": 0.9,
        "fat_per_100": 21.2,
        "carbon_per_100": 9.6,
        "fiber_per_100": 2.8,
        "sugar_per_100": 6.8,
        "salt_per_100": 0.1,
    }
}]

ingredient_data_response = [
    {'ingredient_id': 1},
    {'calories_per_100': 41.6},
    {'protein_per_100': 0.9},
    {'fat_per_100': 21.2},
    {'carbon_per_100': 9.6},
    {'fiber_per_100': 2.8},
    {'sugar_per_100': 6.8},
    {'salt_per_100': 0.1}
]


@pytest.mark.asyncio
async def test_post_nested_elements(mocked_supabase_connection_init):
    await create_element("ingredients", ingredient_create)
    await create_nested("ingredients", 1, ingredient_data_create)
    response = await get_nested("ingredients", 1, ["ingredients_data"])

    assert response == ingredient_data_response


@pytest.mark.asyncio
async def test_post_nested_elements_error(mocked_supabase_connection_init):
    response = await get_nested("ingredients", 1, ["ingredients_data"])

    assert response == [{'ingredients_data': []}]