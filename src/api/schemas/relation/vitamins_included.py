...
# from api.schemas import IngredientDB, VitaminDB
...
class VitaminsIncludedDB(VitaminsIncludedBase, table=True):
    ...
    vitamin_id: int = Field(
        foreign_key=f"{settings.VITAMIN_TABLE}.id", primary_key=True
    )
    ingredient_id: int = Field(
        foreign_key=f"{settings.INGREDIENT_TABLE}.id", primary_key=True
    )
