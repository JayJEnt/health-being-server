...
async def admin_only(user: Annotated[UserResponse, Depends(validate_token)]):
    if user.role != "admin":
        raise DemandAdminAccess


async def logged_only(user: Annotated[UserResponse, Depends(validate_token)]):
    if not user:
        raise DemandBeingLogged


async def owner_only(element_type: str, element_id: int, user: UserResponse):
