...
async def create_relationship(
    element_type: str, element_id: int, relation_name: str, related_data: dict
) -> dict:
    """
    ...
    try:
        exists = await get_element_by_name(
            relation_config["name"], related_data[related_config["column_name"]]
        )
    ...
    if element_type == relation_name and element_id == exists["id"]:
        logger.error(
            f"Element {element_type} with id={element_id} tries to reference to itself."
        )
        raise ReferencesToItself

    try:
        await get_relationship(element_type, element_id, relation_name, exists["id"])
        raise ResourceAlreadyTaken
    except ResourceAlreadyTaken:
        logger.error(
            f"Relation {relation_config['name']} with ids: {element_id}, {exists["id"]} already exists."
        )
        raise
    except ResourceNotFound:
        pass
