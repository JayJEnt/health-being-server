...
from api.schemas.nested.user_data import UserData, UserDataCreate
...
async def get_owner_data(requesting_user: UserResponse = Depends(validate_token)):
...
async def update_owner_data(
    user: UserDataCreate, requesting_user: UserResponse = Depends(validate_token)
):
...
async def delete_owner_data(requesting_user: UserResponse = Depends(validate_token)):
