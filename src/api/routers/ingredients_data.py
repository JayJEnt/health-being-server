...
async def get_ingredient_data(ingredient_id: int):
    return await crud.get_by_id("ingredients_data", ingredient_id)
...
async def create_ingredient_data(ingredient_id: int, ingredient: IngredientDataCreate):
    ...
    return await crud.post("ingredients_data", ingredient)
...
async def update_ingredient_data(ingredient_id: int, ingredient: IngredientDataCreate):
    return await crud.put("ingredients_data", ingredient_id, ingredient)


@admin_router.delete("/{ingredient_id}", dependencies=[Depends(admin_only)])
async def delete_ingredient_data(ingredient_id: int):
    return await crud.delete("ingredients_data", ingredient_id)
